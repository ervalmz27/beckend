<script>
    ...
    metatags.title = 'Group - Buzzstrike';
    metatags.description = 'Buzzstrike Group';
        ...
        Group.get(10).then(d=>{
            ...
            if(d && d.length > 0)  {
                ...
                d.forEach((element, i) => {
                    ...
                    el.isActive = false;
                    // el.members = [];
                    // element.relation('members').query().each((relatedObject) => {
                    //     const contact = relatedObject.toJSON();
                    //     contact.isActive = false;
                    //     el.members.push(contact);
                    // });
                    arr.push(el);
                });
                data = arr;
    ...
    async function removeChecked() {
        ...
        for (let i = 0; i < data.length; i++) {
            if(data[i].isActive) {
                const r = await Group.remove(data[i].objectId);
                if(!r) return false;
            }
        }
        fetchGroup();
    ...
    <div class="left w-full h-auto" >
        <div class="bg-white rounded-full w-auto mr-3 h-14 mt-6 ml-3 flex">
            <div class="w-2/4 flex">
                <div class="border-2 font-xl p-3 h-14 bg-green-500 rounded-l-full text-white w-3/5 text-xl flex">Daftar Intance
                </div>
                <div class="grid grid-flow-col gap-4 w-1/3 ml-auto h-10 mt-4">
                    <svg on:click={fetchGroup} class="w-6 h-6 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="w-2/4 h-full border border-gray-400 rounded-xl pb-4 flex">
                    <div class="w-full rounded-3xl p-0.4">
                        ...
                        <div on:click={()=>bgActive = i} class=" {bgActive == i ? "bg-green-100" : "bg-white hover:bg-gray-200"} group relative wrapper p-2 flex border-b border-gray-400 w-full h-16 rounded-t-xl cursor-pointer">
                            ...
                            <h1 class="px-3 text-gray-500">{datum.name} <br /> {datum.desc ? datum.desc.length > 30 ? datum.desc.slice(0,30) : datum.desc : "-"}</h1>
