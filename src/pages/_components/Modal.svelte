<script>
  ...
  function onClicked(isSubmit = false) {
    if($modal.callback) {
      $modal.callback(isSubmit);
    }
    modal.set({active: false, message: ""});
  }
</script>
