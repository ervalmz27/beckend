<script>
    ...
    metatags.title = 'Kontak - Buzzstrike';
    metatags.description = 'Buzzstrike Contact';
    ...
    import SendContact from './_components/SendContact.svelte';
   
    onMount(fetchContact);
    let data = [];
    ...
    async function removeChecked() {
        ...
        for (let i = 0; i < data.length; i++) {
            if(data[i].isActive) {
                const r = await Contact.remove(data[i].objectId);
                if(!r) return false;
            }
        }
        fetchContact();
    }
    // async function checkFetchContact() {
    //     loading.set(true);
    //     const res = await Contact.getByNumber("v1PPCIjKDa", "6285220802180");
    //     if(res.ok) {
    //         loading.set(false);
    //         return res.json();
    //     } else {
    //         loading.set(false);
    //         modal.set({active: true, message:"Kontak tidak dapat ditemukan! " + "Error: [" + res.status + "] " + res.statusText + ".", ok: true});
    //     }
    // }
    // function checkNumber() {
    //     let promise = checkFetchContact();
    //     promise.then(d=>{
    //         d.contactId = d.id;
    //         d.uid = d.id._serialized;
    //         delete d.id;
    //         Contact.create(d).
    //         then(d=>{
    //             console.log(d)}
    ...
    const searchedPosts = () => {
      return sear = data.filter(d => {
          let searchContact = d.name + d.number;
...
{/if}
{#if send}
    <SendContact context={callbackIm}/>
{/if}
<div class="flex h-full overflow-hidden">
    <div class="left w-full h-auto" >
        <div class="bg-white rounded-full w-auto mr-3 h-14 mt-6 ml-3 flex">
            <div class="w-2/4 flex">
                <div class="border-2 font-xl p-3 h-14 bg-green-500 rounded-l-full text-white w-3/5 text-xl flex">Daftar Kontak
                </div>
                <div class="grid grid-flow-col gap-4 w-1/2 ml-auto  mt-4">
                    <button on:click={()=>send=true} class=" focus:outline-none w-6 h-6 cursor-pointer">
                     <i class="fas fa-file-import"></i>
                    </button>
                    <svg on:click={fetchContact} class="w-6 h-6 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
